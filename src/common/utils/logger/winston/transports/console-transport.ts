...
export default class ConsoleTransport {
    ...
    return new winston.transports.Console({
      format: winston.format.combine(
        winston.format.printf((log) => {
          const color = this.LogLevelColorMapping(log.level as LogLevel);
          ...
          return `${this.colorize(color, prefix + '  -')} ${log.timestamp}  
                  ...
                  ${
                    log.data.stack
                      ? this.colorize(color, `  - ${log.data.stack}`)
                      : ''
                  }
                  ${
                    log.data.props
  ...
  public static colorize(color: LogColors, message: string): string {
    return `${color}${message}\x1b[0m`;
  }

  public static LogLevelColorMapping(level: LogLevel): LogColors {
    switch (level) {
      case LogLevel.Debug:
